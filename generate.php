...
for($x = 1; $x <= $brp; $x++){
    ...
    $uname  = strtolower($fname.$lname).rand(1000, 9999)."@".$domain;
    $format .= $lname.",".$fname.",".$uname.",".$pass.",,/".PHP_EOL;
    $as_text .= $uname.":".$pass.PHP_EOL;
}
$f = fopen("result.csv", "w+");
fwrite($f, $format);
fclose($f);
$s = fopen("as_text.txt", "w+");
fwrite($s, $as_text);
fclose($s);
echo "[~] Success Save user to result.csv".PHP_EOL;
?>
